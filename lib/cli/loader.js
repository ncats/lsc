        ...
        return this._init().then(() => {
            ...
            _.each(this._commands, pkg => {
                _.each(pkg, (modulePath, name) => {
                    ...
                    app.commands[name] = require(modulePath);
                });
            });
        }).nodeify(callback);
    };
        ...
        _.each(this._initModules, initModulePath => {
            promises.push(Q.Promise((resolve, reject) => {
                ...
                try {
                    ...
                    return Q.reject(new Error(`CLI LOAD ERROR: failed to load init module at ${initModulePath}: ${error.stack}`));
                }
                ...
                if (init.length) {
                    ...
                    setTimeout(() => {
                        reject(new Error(`CLI TIMEOUT ERROR: init function in "${initModulePath}" failed to callback within ${this.options.timeout / 1000} seconds`));
