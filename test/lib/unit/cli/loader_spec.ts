...
describe('CliLoader', () => {
    ...
    describe('.load', () => {
        ...
        it('attempts to load commands from each directory specified by the "directories" option',
            async () => {
    ...
    describe('.unload', () => {
        ...
        it('deletes all the commands assigned to the app by .load', async () => {
            ...
            expect(toContainAllOf(_.keys(flatironAppMock.commands),
                ['hello', 'add', 'subtract', 'goodbye'])).toBeTruthy();
            expect(_.keys(cliLoader._commands).length).toBeGreaterThan(0);
            cliLoader.unload();
            expect(toContainAllOf(_.keys(flatironAppMock.commands),
                ['hello', 'add', 'subtract', 'goodbye'])).toBeFalsy();
